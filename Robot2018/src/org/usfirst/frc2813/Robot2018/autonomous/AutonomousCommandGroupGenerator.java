...
public class AutonomousCommandGroupGenerator {
	...
	/** Prepare to shoot a cube at the switch. This is used as a starting position as well. */
	...
	/**
	 * Prepare to shoot a cube at the scale. This starts the elevator moving up all the way!
	 * Be careful!!! We will not be very stable.
	 * 
	 * @param direction - which direction is the robot facing?
	 */
	...
	public AutonomousCommandGroupGenerator() {
		...
		/** This initial state may not be exactly what we want, but it's safe. We can change it later. */
		...
		if (robotStartingPosition.equals(scalePosition)) {
			...
			if (useCurves) {
				...
				autoCmdList.drive.addCurveDegreesSync(Direction.FORWARD, degrees, inches(radius), Direction.COUNTERCLOCKWISE, SPEED_FULL);
				...
				autoCmdList.drive.addDriveSync(Direction.FORWARD, inches(turnClearance), SPEED_TURN); /**  enough to turn */
			...
			if (useCurves) {
				...
				autoCmdList.drive.addDriveSync(Direction.BACKWARD, inches(backWallToScaleAlley + scaleAlleyWidth / 2), SPEED_TURN);
				autoCmdList.drive.addQuickTurnSync(left, 90); /**  right but we're backwards */
